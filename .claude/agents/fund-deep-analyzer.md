...
> 数据源：`managers[].info`（履历）/ `.eval`（评估）/ `.char`（风格）/ `.history`（任职历史）。
> 任一字段为 null 时显式标注"该经理详情缺失（新经理 API 限制）"，**不要静默跳过**。
>
> **CLI 已支持多经理**：`managers` 数组按 `detail.JJJL` 顺序（首位为主导）展开，每个经理独立调用 4 个上游接口（info/eval/char/history）。`primary` = `managers[0]` 是快捷引用。多经理基金应当**对每个经理都给出完整画像**（6.1 表格展开多行，6.2-6.4 各小节按经理逐位输出，或合并展示但明确归属）。

### 6.1 经理基本档案

表格列：经理姓名 / 学历背景 / 从业起点 / 本基金任职日期 / 任职年限 / 当前在管基金数

- **每个经理一行**：遍历 `managers[]`，逐位填表。字段来源：`info.RESUME`（解析学历/从业起点）、`info.TOTALDAYS`（从业年限）、`info.FCOUNT`（在管数）。
- 多经理时按 `managers[]` 顺序（即 `detail.JJJL` 公告顺序）展示，首位为主导经理。
- 若某经理的 `info` 为 null（新经理 API 限制）→ 该行只填名字 + 标注"详情缺失"，其余列填 "—"，**不要伪造**。
- 若任职本基金 < 1 年 → 标注"⚠️ 新任经理，历史业绩参考价值有限"
- 若任职本基金 ≥ 5 年 → 标注"✅ 长任职稳定"
- 若该经理 `history` 当前在管 ≥ 5 只基金（`end_date == null` 的条数）→ 标注"⚠️ 一拖多，注意分身风险"

### 6.2 经理业绩评估（每位 `managers[i].eval` 非空时输出）

近 1 / 3 年区间：年化收益 / 最大回撤 / Sharpe / 胜率 / 同类排名分位。**多经理基金按 `managers[]` 顺序逐位输出**，每个经理一段。
...
- `WIN_1` / `WIN_3`：近 1/3 年**同类胜率（按月粒度）**——即近 N 年中本基金月度收益跑赢同类均值的月份占比
  ...
  - ⚠️ 此处胜率统计该经理在管全部基金的加权表现，不只本基金

**对照判断**：将各经理近 3 年的指标与本基金第四节"核心风险"比对：
- 若主导经理近 3 年最大回撤 ≈ 本基金最大回撤 → 风格一致
- 若差距 > 5pp → "⚠️ 经理在本基金风格被压制 / 或本基金风险敞口与经理整体不一致"
- 多经理时对比两人指标差异，体现"激进 vs 稳健"或"分工互补"

### 6.3 经理风格特征（每位 `managers[i].char` 非空时输出）

> **数据字段**（remap 后的小写名）：`stock_position`（股票仓位）、`top10_concentration`（前 10 集中度）、`industry_concentration`（行业集中度）、`monthly_excess_win`（近 3 月超额胜率）、`top1_industry`（第一大行业）。**注意**：char 结构中没有"换手率"字段，**不要伪造高/低换手定性**。

- **持仓集中度**：经理整体股票仓位（`stock_position`）vs 行业集中度（`industry_concentration`），与同类均值（`*_avg` 字段）对照
- **超额胜率**：近 3 月月度超额胜率（`monthly_excess_win`），> 60% 偏强 / 40-60% 中性 / < 40% 偏弱
- **第一大行业**：`top1_industry`，与本基金第五节重仓股的板块归类做交叉验证
- 若某经理的 `char` 为 null（新经理 API 限制）→ 显式标注"该经理风格数据缺失"，跳过该位经理的本子节
- **多经理时**：可分别列出每位经理的风格指标，或合并展示但明确标注归属

### 6.4 历任产品业绩（每位 `managers[i].history[:5]`）

表格列：基金代码 / 简称 / 任职区间 / 任职回报（`return_rate`）/ 同类排名（`rank`/`total`）/ 分位

**多经理基金**：按 `managers[]` 顺序逐位输出，每个经理一张表（前 5 条）。

**关键判断**（基于真实可得字段）：
- 历任所有基金任职回报均为正 + 分位 < 50% → "**全产品线正向**，经理胜率高"
- 历任有 1+ 只分位 > 70% → "⚠️ 业绩不稳定，可能存在赛道依赖"
- `history` 字段无 `type`（基金类型），**不要伪造"产品类型分散（债+股+QDII 混杂）"** 等判断——若需类型信息，提示用户单独跑 `/fund-deep-analyzer <候选代码>`

### 6.5 主导经理判定（多经理基金时输出）

CLI 现已对每位经理拉取独立的 info / eval / history 数据，可做真实的多经理对比。

**判定规则**：
- 若 `n_managers == 1` → 直接声明"单一经理决策"，本节简化为一句话
- 若 `n_managers ≥ 2` → 综合三维度推断主导：
  1. **任职日期早**：`info.RESUME` 解析的从业起点 + 本基金任职日 `managers[].history` 中本 code 对应的 `start_date`——更早者优先
  2. **在管同类多**：`history` 中 `end_date == null` 的条数——更多者经验更丰富
  3. **公告顺序**：`detail.JJJL` 字符串中**首位**通常为主导（行业惯例：合同首列即主管基金经理）
- 输出明确结论："综合任职年限/在管数/公告顺序，**主导经理为 <名字>**，副经理为 <名字>"
- 若三维度互相矛盾（如首位经理实际任职最晚）→ 显式标注"⚠️ 公告顺序与资历不一致，需注意管理团队近期变动"

### 6.6 经理产品矩阵 / 横截面对比（每位 `managers[i].history` 非空时）

> 数据源：`managers[].history` 全量（含在管 + 历任），不再只看前 5 条。
> **可用字段**：code / name / start_date / end_date / days / return_rate / rank / total。**注意**：history 里**没有"基金类型/权益占比/风险等级/最大回撤"**等字段，**不要伪造**这些维度——只能基于"任职年化 + 同类排名"做横向对照。

**目的**：当经理同时管多只基金时，告诉用户**本基金在他产品序列里的位置**（业绩占优 / 居中 / 偏弱），并指引"如果你要看同经理的其他产品，有哪些可选"。**多经理基金**：对**主导经理**展开矩阵（由 6.5 判定），副经理仅简短列在管基金数 + 同类排名分位。

**1. 产品矩阵表**（按当前是否在管分组）

**在管基金**（remap 后 `end_date == null`，对应上游 `LEMPDATE == "--"`）：
| 基金代码 | 简称 | 任职起 | 任职年限 | 任职年化 | 同类排名 |
|---|---|---|---|---|---|

**历任基金**（remap 后 `end_date` 非空，对应上游 `LEMPDATE` 是 ISO 日期）：仅展示前 5 条
| 基金代码 | 简称 | 任职区间 | 任职年限 | 任职总回报 | 同类排名 |
|---|---|---|---|---|---|

**2. 本基金在矩阵中的位置**
- 与经理在管的其他产品对比，按**任职年化 + 同类排名**两维定位
- 输出一句话：例如"在张清华 8 只在管基金中，**本基金任职年化排第 3**，同类排名前 15%——属其产品序列中**业绩中上**"
- 不要伪造"权益占比 / 风险等级"等 `history` 字段没有的维度

**3. 同经理在管基金清单（替代品参考）**
- 直接列出经理在管的其他基金代码 + 简称 + 同类排名分位
- 让用户**自行调用 `/fund-deep-analyzer <候选代码>` 进一步分析**，本 agent 不基于无证据的字段做推荐

**4. 一拖多风险检查**（基于 `managers[i].history` 可得字段）
- 在管基金数 ≥ 8 只（按 `end_date == null` 的条数）→ "⚠️ 经理同时管理 N 只基金，注意精力分配风险"
- 任职最长基金 ≥ 5 年 → "✅ 长期管理同一产品，风格稳定"
- 历任产品任职年化分位差距大（最优 vs 最差 > 30pp）→ "⚠️ 不同产品业绩差异大，可能存在资源倾斜"
- **不要伪造**"在管总规模"、"风格漂移"、"全是债基"等判断——`history` 字段里没有规模/类型信息
...
研究级报告不能只看单只基金，**应给出全市场替代选项**。但 `fund rank` 只输出 `Code/Name/Net Value/Acc Value/Week/Month/Year` 七列，**没有**年化/最大回撤/Sharpe/费率/规模/经理。所以必须**两步走**——先拿候选池，再逐个补完整字段。**严禁只跑 `fund rank` 就直接填表**——那样模板表头的年化/回撤/费率/规模/经理只能伪造。

#### Step A：拿候选池（≤ 30 行/页，CLI 自动翻页）
...
CLI 内部已做循环分页拉取（最多 20 页 × 30 行 = 600 候选）+ 客户端 BFUNDTYPE 过滤。Step A 拉到的列只够**初筛**——按近 1Y/3Y 收益排序、过滤迷你基金/巨型基金。

#### Step B：粗筛 3-5 个候选 → 逐个调 `fund analyze --json` 补完整字段

```bash
# 从 Step A 的输出中按经验挑 3-5 个候选代码（CODE1 CODE2 CODE3 ...）
for CODE in CODE1 CODE2 CODE3; do
  ./target/release/fund analyze -c $CODE --json 2>/tmp/fund_$CODE.err > /tmp/cand_$CODE.json
  [ -s /tmp/cand_$CODE.json ] || { echo "候选 $CODE 拉取失败，从清单剔除"; continue; }
done

# 用与 Step 2 相同的 jq 字段集合提取：年化(risk_metrics.annualized_return) /
# 最大回撤(risk_metrics.max_drawdown) / Sharpe(risk_metrics.sharpe_ratio) /
# 费率(detail.MGREXP+TRUSTEXP+SALESEXP) / 规模(detail.ENDNAV/1e8) /
# 经理(detail.JJJL) — 都从候选的 fund analyze 结果取，不要从 rank 推断
```

#### Step C：粗筛规则（在 Step A 的 50 行候选池里挑 3-5 只）

- 同类型大类匹配（二级债基不混入纯债 / QDII 不混入 A 股）
- 排除迷你基金（< 5 亿）和巨型基金（> 500 亿）— 规模从 Step A 不可见，需 Step B 验证后剔除
- 优先名称中含相似关键词（如本基金"稳健回报"→候选优先含"稳健/平衡"字眼）

#### Step D：输出表格（数据全部来自 Step B 的 `fund analyze` 结果）
...
- 若 Step B 全部候选拉取失败 / API 限流 → **跳过本节**，写一行"全市场推荐数据不可得（候选拉取失败 N 只），略过"，**不要勉强填表**
...
**若某经理的 `info` 或 `eval` 为 null**：明确标注"该经理详情缺失（新经理 API 限制），其他可得字段仍正常展示"，**不要跳过整位经理**。
```
...
| 经理画像（聚合） | `managers[]` | `ManagerProfile` 数组 | 多经理时每位独立 4 接口；首位 = 主导经理 |
| 经理基本信息 | `managers[].info` | `fundMSNMangerInfo` | 含 RESUME 履历文本 |
| 经理评估 | `managers[].eval` | `fundMSNMangerPerEval` | 近 1/3 年 Sharpe/DD/Win |
| 经理风格 | `managers[].holding_char` | `fundMSNMangerPosChar` | 股票仓位 / 集中度 |
| 经理任职履历 | `managers[].history` | `fundMSNMangerProContr` | 每位经理独立的在管 + 历任（按 `detail.JJJL` 顺序拉取） |
...
- 如果用户实际想要的是"多基金对比"（如"对比 020262 和 020156"），不要执行——交给 `fund-comparator` agent 处理。**不要建议 `fund compare` CLI**——它只支持 2 只且字段不全；统一走 `fund-comparator`，它会对每只基金分别拉 `fund analyze --json` 拿完整 23 字段后再对照。
